...
def _guarded_hint_for_unbacked_int(size: IntLikeType) -> IntLikeType:
    from torch.fx.experimental.symbolic_shapes import (
        free_unbacked_symbols,
        optimization_hint,
    )
    ...
    if not size.node.expr.free_symbols:
        return int(size)

    if not (
        (shape_env := getattr(size.node, "shape_env", None)) is not None
        and (unbacked_symbols := free_unbacked_symbols(size.node.expr))
        and unbacked_symbols.issubset(shape_env.var_to_hint_override.keys())
    ):
        return size

    hint = optimization_hint(size, fallback=None)
    torch._check(size == hint)
    return hint
...
class _StridedShard(torch._C._distributed.StridedShard):
    ...
    def _to_replicate_tensor(
        ...
        max_chunk_size = _guarded_hint_for_unbacked_int(max_chunk_size)
        local_pad_size = max_chunk_size - local_tensor.size(self.dim)
        local_pad_size = _guarded_hint_for_unbacked_int(local_pad_size)
        ...
        logical_shape = cast(list[int], list(current_logical_shape))
        replicate_tensor = replicate_tensor.as_strided(
            logical_shape,
            torch._prims_common.make_contiguous_strides_for(logical_shape),
        )
