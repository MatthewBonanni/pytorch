...
// A backend producing duplicate async-CPU-GPU flow start IDs must not crash
// the profiler. Pre-fix this hit TORCH_INTERNAL_ASSERT in
// TransferEvents::setParents(); post-fix it must warn and complete.
TEST(ProfilerCollectionTest, DuplicateFlowIdHandledGracefully) {
  ...
  // Pre-fix this call crashed inside TransferEvents::setParents() on the
  // duplicate flow start ID. Post-fix it returns normally.
